...
- 段落と段落の間には必ず「空行（二重改行）」を入れてください。単なる改行だけでは表示が崩れます。
- 見出し（###）や水平線（---）、リスト（*）の前後には必ず「空行」を入れてください。これらが文章と地続きになると正しく表示されません。
- 強調したい単語は必ず二重のアスタリスク（例：**重要**）で囲んでください。単一のアスタリスク（*）は絶対に使用しないでください。
- 改行が必要な場合は、行末に半角スペース2つを入れるか、空行（二重改行）を適切に使用してください。
`;
